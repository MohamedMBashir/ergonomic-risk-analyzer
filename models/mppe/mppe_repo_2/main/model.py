...
from models.mppe.mppe_repo.common.nets.resnet import ResNetBackbone
...
def soft_argmax(heatmaps, joint_num):
    heatmaps = heatmaps.reshape(
        (-1, joint_num, cfg.depth_dim * cfg.output_shape[0] * cfg.output_shape[1])
    )
    ...
    def forward(self, input_img, target=None):
        ...
        if target is None:
            ...
            loss_coord = (
                loss_coord[:, :, 0]
                + loss_coord[:, :, 1]
                + loss_coord[:, :, 2] * target_have_depth
            ) / 3.0
