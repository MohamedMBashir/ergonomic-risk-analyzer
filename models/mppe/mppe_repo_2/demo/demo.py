...
def vis_3d_multiple_skeleton_with_rula_angles(
    kpt_3d, kpt_3d_vis, kps_lines, rula_angles, filename=None
):
    ...
    # Add RULA angle annotations with more detailed labels
    for n in range(person_num):
        ...
        ax.text(
            ...
            shoulder[2],
            -shoulder[1],
            f"Upper Arm\n(Shoulder-Torso): {person_angles['upper_arm']:.1f}°",
            fontsize=8,
            color="red",
            ha="center",
            va="bottom",
        ...
        ax.text(
            ...
            elbow[2],
            -elbow[1],
            f"Lower Arm\n(Elbow angle): {person_angles['lower_arm']:.1f}°",
            fontsize=8,
            color="blue",
            ha="center",
            va="bottom",
        ...
        ax.text(
            ...
            wrist[2],
            -wrist[1],
            f"Wrist\n(Wrist-Lower Arm): {person_angles['wrist']:.1f}°",
            fontsize=8,
            color="green",
            ha="center",
            va="bottom",
        ...
        ax.text(
            ...
            neck[2],
            -neck[1],
            f"Neck\n(Head-Torso): {person_angles['neck']:.1f}°",
            fontsize=8,
            color="purple",
            ha="center",
            va="bottom",
        ...
        ax.text(
            ...
            trunk[2],
            -trunk[1],
            f"Trunk\n(Torso-Vertical): {person_angles['trunk']:.1f}°",
            fontsize=8,
            color="orange",
            ha="center",
            va="bottom",
        ...
        ax.text(
            ...
            pelvis[2],
            -pelvis[1],
            f"Legs\n(Binary Score): {person_angles['leg']}",
            fontsize=8,
            color="brown",
            ha="center",
            va="bottom",
    ...
    if filename is None:
        ax.set_title("3D visualization with RULA angles", fontsize=16)
    else:
...
def calculate_rula_angles(pose_3d):
    ...
    # Define vectors
    torso_vector = (
        pose_3d[JOINTS_NAME.index("Pelvis")] - pose_3d[JOINTS_NAME.index("Thorax")]
    )
    vertical_vector = np.array(
        [0, -1, 0]
    )  # Assuming Y is vertical in your coordinate system
    ...
    upper_arm_angle = (
        angle_between(upper_arm_vector, torso_vector) - 90
    )  # Adjust to RULA convention
    ...
    leg_score = (
        0 if abs(left_ankle[1] - right_ankle[1]) < 10 else 1
    )  # Threshold of 10mm
...
def main():
    ...
    img_path = "./inputs/rula_test_girl.jpg"
    ...
    output_pose_2d_list, output_pose_3d_list = process_image(
        model, original_img, transform, bbox_list, root_depth_list
    )

    # Calculate RULA angles for all persons
    all_rula_angles = [
        calculate_rula_angles(pose_3d) for pose_3d in output_pose_3d_list
    ]
