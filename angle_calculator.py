...
class AngleCalculator:
    ...
    def calculate_angles(self, keypoints_dict):
        ...
        # Add visualization
        self.visualize_keypoints(keypoints, side_keypoints, "Original vs Rotated Keypoints (Side View)")
        ...
        trunk_bended = 1 if abs(trunk_bend_angle) > 10 else 0  # If trunk bends sideways more than 10 degrees
        ...
        # Calculate and print adjustment angles
        print("\nAdjustment Angles:")
        print(f"Shoulder Raise Angle: {shoulder_angle:.2f}° (Raised: {shoulder_raised})")
        print(f"Arm Abduction Angle: {arm_abduction_angle:.2f}° (Abducted: {upper_arm_abducted})")
        print(f"Neck Bend Angle: {neck_bend_angle:.2f}° (Bent: {neck_bended})")
        print(f"Trunk Bend Angle: {trunk_bend_angle:.2f}° (Bent: {trunk_bended})")
        
        # Print arm outside/inside calculation details
        right_hand = top_keypoints[keypoints_names.index('right_hand')]
        right_elbow = top_keypoints[keypoints_names.index('right_elbow')]
        right_shoulder = top_keypoints[keypoints_names.index('right_shoulder')]
        left_shoulder = top_keypoints[keypoints_names.index('left_shoulder')]
        
        arm_bend_angle = self.calculate_angle_between(right_hand, right_elbow, right_shoulder)
        shoulder_arm_angle = self.calculate_angle_between(left_shoulder, right_shoulder, right_elbow)
        combined_angle = 180 - (arm_bend_angle + shoulder_arm_angle)
        
        print("\nArm Outside/Inside Analysis:")
        print(f"Arm Bend Angle (hand-elbow-shoulder): {arm_bend_angle:.2f}°")
        print(f"Shoulder-Arm Angle (left_shoulder-right_shoulder-right_elbow): {shoulder_arm_angle:.2f}°")
        print(f"Combined Deviation: {combined_angle:.2f}° (Outside: {arms_outside})")
    ...
    def _get_trunk_bend_angle(self, keypoints):
        ...
        trunk_angle = self.calculate_angle_between(upper_torso, bottom_torso, vertical_point)
    ...
    def _get_arm_abduction_angle(self, keypoints):
        ...
        abduction_angle = self.calculate_angle_between(right_elbow, right_shoulder, upper_torso)
    ...
    def visualize_keypoints(self, original_keypoints, rotated_keypoints, title="Keypoint Visualization"):
        """
        Visualize keypoints in four views:
        1. Original 3D view
        2. Side view (Y-Z plane)
        3. Front view (X-Z plane)
        4. Top view (X-Y plane)
        """
        fig = plt.figure(figsize=(20, 10))
        
        # Original keypoints subplot (3D)
        ax1 = fig.add_subplot(221, projection='3d')
        self._plot_skeleton(ax1, original_keypoints, "Original Keypoints", dim='3d')
        
        # Side view
        ax2 = fig.add_subplot(222)
        self._plot_skeleton(ax2, rotated_keypoints, "Side View (Y-Z)", dim='side')
        
        # Front view
        ax3 = fig.add_subplot(223)
        self._plot_skeleton(ax3, rotated_keypoints, "Front View (X-Z)", dim='front')
        
        # Top view
        top_keypoints = self.rotate_keypoints(original_keypoints, view="top")
        ax4 = fig.add_subplot(224)
        self._plot_skeleton(ax4, top_keypoints, "Top View (X-Y)", dim='top')
        
        plt.suptitle(title)
        plt.tight_layout()
        plt.show()

    def _plot_skeleton(self, ax, keypoints, subtitle, dim='3d'):
        """Helper function to plot keypoints and connections"""
        if dim == '3d':
            # 3D plot for original keypoints
            ...
            # 2D plot for side view (Y-Z plane)
            ax.scatter(keypoints[:, 1], keypoints[:, 2], c='blue', marker='o')
            ...
            # 2D plot for front view (X-Z plane)
            ax.scatter(keypoints[:, 0], keypoints[:, 2], c='blue', marker='o')
            ...
            # 2D plot for top view (X-Y plane)
            ax.scatter(keypoints[:, 0], keypoints[:, 1], c='blue', marker='o')
